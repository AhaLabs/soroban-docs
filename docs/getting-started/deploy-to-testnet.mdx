...
const greetingTx = await greeter.hello({ to: "Soroban" });
 ---
```
...
```typescript title="src/pages/index.astro"
<h1>
  <span class="text-gradient">{greetingTx.result.join(' ')}</span>
</h1>
```
...
In this case, our `greeter.hello({ to: 'Soroban' });` method is making an RPC call on the server, and the response from that call is being injected onto our index page where we put `{greetingTx.result.join(' ')}`. For information about RPC calls, checkout out the [RPC](../reference/rpc.mdx) reference page.
